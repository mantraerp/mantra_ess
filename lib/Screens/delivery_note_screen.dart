...
class _DeliveryNoteListScreenState extends State<DeliveryNoteListScreen> {
  ...
  Future<void> _fetchDeliveryNotes({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final poData = data["data"]?["delivery_note"] ?? [];
        ...
        setState(() {
          final existingNames = _deliverynotes.map((e) => e.name).toSet();
          final newItems =
          poResponse.data.where((e) => !existingNames.contains(e.name)).toList();

          if (isRefresh) {
            _deliverynotes = poResponse.data;
          } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : RefreshIndicator(
        onRefresh: () => _fetchDeliveryNotes(isRefresh: true),
        child: ListView.builder(
          controller: _scrollController,
          itemCount: _deliverynotes.length + (_isPaginating ? 1 : 0),
            ...
            return Card(
              ...
              child: InkWell(
                ...
                onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        DeliveryNoteDetailScreen(deliveryNoteName: dn.name),
                  ),
                ),
                child: Padding(
