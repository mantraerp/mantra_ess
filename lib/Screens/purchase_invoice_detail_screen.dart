...
class PurchaseInvoiceDetailScreen extends StatefulWidget {
  final String purchaseInvoiceName;

  const PurchaseInvoiceDetailScreen({Key? key, required this.purchaseInvoiceName})
      : super(key: key);
...
class _PurchaseInvoiceDetailScreenState extends State<PurchaseInvoiceDetailScreen> {
  ...
  Future<void> _fetchActivityLog() async {
    ...
    try {
      ...
      final doctype = "Purchase Invoice";
      final response = await http.get(
        Uri.parse(
            "$GetActivityLogs?doctype=$doctype&name=${widget.purchaseInvoiceName}"),
        headers: headers,
      );
  ...
  Future<void> _fetchPurchaseInvoiceDetail() async {
    setState(() {
      _isLoading = true;
      _hasError = false;
    });
    ...
    try {
      final sid = box.read('SID');
      final url = Uri.parse(
          "$GetPurchaseInvoiceDetail?purchase_invoice=${widget.purchaseInvoiceName}");
        ...
        child: Column(
          ...
          children: [
            ...
            _navCard(
              context,
              title: "Items",
              subtitle: "View all items in this Purchase Invoices",
            ...
            BaseActivityScreen(
              ...
              docname:  widget.purchaseInvoiceName,
            ),
