...
class PurchaseReceiptDetailScreen extends StatefulWidget {
  final String purchaseReceiptName;

  const PurchaseReceiptDetailScreen({Key? key, required this.purchaseReceiptName})
      : super(key: key);
...
class _PurchaseReceiptDetailScreenState extends State<PurchaseReceiptDetailScreen> {
  ...
  Future<void> _fetchActivityLog() async {
    ...
    try {
      ...
      final doctype = "Purchase Receipt";
      final response = await http.get(
        Uri.parse(
            "$GetActivityLogs?doctype=$doctype&name=${widget.purchaseReceiptName}"),
        headers: headers,
      );
  ...
  Future<void> _fetchPurchaseReceiptDetail() async {
    setState(() {
      _isLoading = true;
      _hasError = false;
    });
    ...
    try {
      final sid = box.read('SID');
      final url = Uri.parse(
          "$GetPurchaseReceiptDetail?purchase_receipt=${widget.purchaseReceiptName}");
        ...
        child: Column(
          ...
          children: [
            ...
            _navCard(
              context,
              title: "Items",
              subtitle: "View all items in this Purchase Receipts",
            ...
            BaseActivityScreen(
              ...
              docname:  widget.purchaseReceiptName,
            ),
