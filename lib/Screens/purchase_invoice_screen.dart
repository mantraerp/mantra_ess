...
class _PurchaseInvoiceListScreenState extends State<PurchaseInvoiceListScreen> {
  ...
  Future<void> _fetchPurchaseInvoices({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final poData = data["data"]?["purchase_invoices"] ?? [];
        ...
        setState(() {
          final existingNames = _purchaseInvoices.map((e) => e.name).toSet();
          final newItems =
          poResponse.data.where((e) => !existingNames.contains(e.name)).toList();

          if (isRefresh) {
            _purchaseInvoices = poResponse.data;
          } else {
            _purchaseInvoices.addAll(newItems);
          }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : RefreshIndicator(
        onRefresh: () => _fetchPurchaseInvoices(isRefresh: true),
        child: ListView.builder(
          controller: _scrollController,
          itemCount: _purchaseInvoices.length + (_isPaginating ? 1 : 0),
            ...
            return Card(
              ...
              child: InkWell(
                ...
                onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        PurchaseInvoiceDetailScreen(purchaseInvoiceName: dn.name),
                  ),
                ),
                child: Padding(
