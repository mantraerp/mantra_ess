...
class PurchaseReceiptListScreen extends StatefulWidget {
  ...
  @override
  State<PurchaseReceiptListScreen> createState() => _PurchaseOrderListScreenState();
}

class _PurchaseOrderListScreenState extends State<PurchaseReceiptListScreen> {
  ...
  Future<void> _fetchPurchaseReceipt({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final poData = data["data"]?["purchase_receipts"] ?? [];
        ...
        setState(() {
          final existingNames = _purchasereceipt.map((e) => e.name).toSet();
          final newItems =
          poResponse.data.where((e) => !existingNames.contains(e.name)).toList();

          if (isRefresh) {
            _purchasereceipt = poResponse.data;
          } else {
            _purchasereceipt.addAll(newItems);
          }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          child: Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: "Search purchase receipts...",
      ...
      body: _isLoading
          ...
          : RefreshIndicator(
        onRefresh: () => _fetchPurchaseReceipt(isRefresh: true),
        child: ListView.builder(
          controller: _scrollController,
          itemCount: _purchasereceipt.length + (_isPaginating ? 1 : 0),
            ...
            return Card(
              ...
              child: InkWell(
                ...
                onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        PurchaseReceiptDetailScreen(purchaseReceiptName: dn.name),
                  ),
                ),
                child: Padding(
